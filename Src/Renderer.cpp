...
bool Renderer::Initialize(float screenWidth, float screenHeight)
{
	...
	mWindow = SDL_CreateWindow("Game Programming in C++", 100, 100, static_cast<int>(mScreenWidth), static_cast<int>(mScreenHeight), SDL_WINDOW_OPENGL);
	if (!mWindow)
	{
...
bool Renderer::LoadShaders()
{
	...
	if (!mSpriteShader->Load("Shaders/Sprite.vs", "Shaders/Sprite.ps"))
	{
		return false;
	}
	mSpriteShader->SetActive();
	...
	if (!mMeshShader->Load("Shaders/Phong.vs", "Shaders/GBufferWrite.ps"))
	{
		return false;
	}
	mMeshShader->SetActive();
	...
	if (!mSkinnedShader->Load("Shaders/Skinned.vs", "Shaders/GBufferWrite.ps"))
	{
		return false;
	}
	mSkinnedShader->SetActive();
	...
	if (!mGGlobalShader->Load("Shaders/GBufferGlobal.vs", "Shaders/GBufferGlobal.ps"))
	{
		return false;
	}
	...
	if (!mGPointLightShader->Load("Shaders/BasicMesh.vs", "Shaders/GBufferPointLight.ps"))
	{
		return false;
	}
	// Set sampler indices
